...
- User seed meets the labsheet's Section 5.3 minimums: 4 active Requesters, 1 inactive Requester (fixed 2026-08-25, PR #15 review — the seed originally shipped with only 2 Requesters and no inactive one), plus IT Staff and Administrator. `server/prisma/verify-seed-lab2.ts` checks these counts. `User.isActive` is what the Development Requester selector endpoint (Issue #12/#13) will filter on to keep the inactive Requester out of the dropdown; that endpoint doesn't exist on this branch yet.
